...
{
    public class ProductService : EntityBaseRepository<ProductModel>, IProductService
    {
        private readonly IFileService _fileService;

        
        public ProductService(BaseDbContext context, IFileService fileService) : base(context)
        {
            _fileService = fileService;
        }

        private async Task<string> Upload(ProductModel model, IFormFile file)
        {
            try
            {	// 여러개의 파일일 경우 하나씩 저장
                    // 저장될 경로
        ...
        Task<string> IProductService.Upload(ProductModel model, IFormFile file)
        {
